...
import { Monitor, Mail, Eye, EyeOff } from "lucide-react";
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.exists && !data.hasPassword) {
        toast.warning("No password set for this account (password setup was skipped). Please sign in using OTP. Sending code...");
  ...
  return (
    <div className="relative flex min-h-dvh flex-col items-center justify-center bg-background px-4">
      ...
      <div className="w-full max-w-[400px]">
        ...
        {step !== "otp" && (
          <div id="title-signin" className="mb-7 text-center auth-enter auth-enter-delay-1">
            <h1 className="text-3xl font-bold tracking-tight text-gradient font-display">Sign in to Dento</h1>
            <p className="mt-2 text-sm text-muted-foreground">
              Don't have an account?{" "}
              <button onClick={onSwitchToSignUp} className="font-semibold text-foreground hover:text-primary hover:underline cursor-pointer">
                Sign up
              </button>
            </p>
          </div>
          ...
          <section id="step-email" className="auth-enter auth-enter-delay-2">
            ...
            <form id="email-form" onSubmit={handleEmailSubmit} className="space-y-4">
              ...
              <button
                ...
                className="h-12 w-full rounded-md bg-primary text-sm font-bold text-primary-foreground shadow-sm transition-all hover:bg-primary/90 cursor-pointer active:scale-[0.96] disabled:opacity-60 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={isSubmitting}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-md border border-border bg-card text-sm font-semibold text-muted-foreground transition-all hover:bg-muted hover:text-foreground cursor-pointer active:scale-[0.96] disabled:opacity-60 disabled:cursor-not-allowed"
              >
        ...
        {step === "password" && (
          <section id="step-password" className="auth-enter auth-enter-delay-1">
            <form id="password-form" onSubmit={handlePasswordSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="pw-email" className="text-sm font-semibold text-muted-foreground">Enter your email</Label>
                <Input
                  id="pw-email"
                  type="email"
                  autoComplete="email"
                  disabled
                  value={email}
                  className="h-11 w-full rounded-md border border-border bg-muted/30 px-3.5 text-sm opacity-80 cursor-not-allowed"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="h-12 w-full rounded-md bg-primary text-sm font-bold text-primary-foreground shadow-sm transition-all hover:bg-primary/90 active:scale-[0.96] disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
              >
              ...
              <button
                ...
                onClick={handleTriggerOTP}
                disabled={isSubmitting}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-md border border-border bg-card text-sm font-semibold transition-all hover:bg-muted cursor-pointer active:scale-[0.96] disabled:opacity-60 disabled:cursor-not-allowed"
              >
                <svg className="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="2" y="4" width="20" height="16" rx="2"></rect>
                  <path d="m22 7-10 5L2 7"></path>
                </svg>
                Log in with OTP
              ...
              <button
                ...
                className="text-sm font-semibold text-foreground hover:text-primary hover:underline block mx-auto text-center mt-2 cursor-pointer"
              >
          ...
          <section id="step-otp" className="auth-enter">
            ...
            <form id="otp-form" onSubmit={handleOTPSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="mx-auto block h-11 rounded-md bg-primary px-6 text-sm font-bold text-primary-foreground shadow-sm transition-all hover:bg-primary/90 active:scale-[0.96] disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
              >
        ...
        {step !== "otp" && (
          <div id="footer" className="auth-enter auth-enter-delay-3">
            ...
            <p className="mt-5 text-center text-sm text-muted-foreground">
              Need help?{" "}
              <a href="/help" className="font-semibold text-foreground hover:underline">
                Contact support
              </a>
            </p>
          </div>
